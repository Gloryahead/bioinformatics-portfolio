...
mkdir -p \
    ...
    "$mm10_genome_dir" # Ensure genome directory exists
...
if [ ! -f "$mm10_fasta" ]; then
    echo "ERROR: Genome FASTA file not found at $mm10_fasta. Please place it there."
    exit 1
fi
echo "Genome FASTA file found: $mm10_fasta."


# --- Step 0: Download Genome Annotation (if not already present) ---
if [ ! -f "$gene_annotation" ]; then
    echo "--- Downloading genome annotation (gencode.vM25.annotation.gff3.gz)... ---"
